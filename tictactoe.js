(function ($, console) {
    ...
    var game = {
        ...
        compTurn: compTurn, // compTurn
        disableSpace: disableSpace,
        marks: marks, // Returns array showing where all player or comp marks are.
        checkWin: checkWin,
        over: over
    ...
    function updatePoints() {
        $('#player-points').html(score.player);
        $('#comp-points').html(score.comp);
        $('#tie-points').html(score.tie);
    }
    ...
    function init() {
        ...
        if (game.gameNum > 0) {
            setTimeout(function () {
                $('.square .content h2').html('').removeClass();
            }, 2000)
        }
        game.gameNum++;
    ...
    function playerTurn() {
        ...
        $(allSquares).one('click', function () {
            var iconClass = game.player === 'X' ? 'icon-x' : 'icon-o';

            $(this).find('.content h2').html(game.player).addClass(iconClass);
    ...
    function compTurn() {
        ...
        function draw(id) {
            ...
            var iconClass = game.comp === 'X' ? 'icon-x' : 'icon-o';
            $(id).find('.content h2').html(game.comp).addClass(iconClass);
        }
        ...
        function didPlayerMove(loc) {
            ...
            if (typeof loc === 'string') {
                ...
                for (var key in loc) {
                    if (loc[key] === lastMove) {
                        logger('from didPlayerMove : ' + loc[key]);
                        return true;
                    }
                }
            }
        ...
        if ('corner' === game.path) {
            ...
            if (!game.playerCenterPath) {
                if (moveNum === 5) {
                    if (doWin() !== false) {
                        logger('winning at move 5 because' + doWin());
                        draw(doWin());
                    } else if (typeof block() === 'number') {
                        logger('block at move 5 because' + block());
                        draw(game.board[block()]);
                        ...
                        logger('playerBlocked from move 5');
                        var lastComp = game.lastCompMoveId;
                        logger('lastcomp is from move 5 ' + lastComp);
                        function blank(id) {
                        ...
                        logger('random at move 5 because' + block() + doWin());
                        draw(game.freeMoves[random(game.freeMoves.length)]);
                    }
                }
                if (moveNum === 7) {
                    if (doWin() !== false) {
                        logger('winning at move 7 because' + doWin());
                        draw(doWin());
                    } else if (typeof block() === 'number') {
                        logger('blocking at move 7 because' + block());
                        draw(game.board[block()]);
                    } else {
                        logger('random at move 7 because' + block() + doWin());
                        draw(game.freeMoves[random(game.freeMoves.length)]);
                    }
                }
                if (moveNum === 9) {
                    if (doWin() !== false) {
                        logger('winning at move 9 because' + doWin());
                        draw(doWin());
                    } else if (typeof block() === 'number') {
                        logger('blocking at move 9 because' + block());
                        draw(game.board[block()]);
                    } else {
                        logger('random at move 9 because' + block() + doWin());
                        draw(game.freeMoves[random(game.freeMoves.length)]);
                    }
                }
            } // player !center
        ...
        function getPrevMove() {
            ...
            for (var i = 0; i < game.board.length; i++) {
                if (drawnInSquare(i) && !inPrevMoves(i)) {
                    logger('from get prev move: ' + game.board[i]);
                    return game.board[i];
                }
            }
        }
    ...
    function disableSpace(lastMove, remaining) {
    ...
    function over(type) {
        if (type === 'player wins') {
            score.player++;
            logger('from game.over, player wins');
        } else if (type === 'computer wins') {
            score.comp++;
            logger('from game.over, computer wins');
        } else if (type === 'tie') {
            score.tie++;
            logger('from game.over, tie');
        }
        game.init();
    }
